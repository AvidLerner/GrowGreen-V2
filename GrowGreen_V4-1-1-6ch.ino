...
// comment out RESET_EEPROM after first compile and upload again
// #define RESET_EEPROM  // remove note to reset EEPROM's during setup   run first time and then comment out
#define ON 0x1
#define OFF 0x0
...
// int Val[MAX_CHANNELS] = {0,0,0,0,0,0};
...
void ReadDS3231()
{
  ...
  hr = MyDateTime.Hour;
  mn = MyDateTime.Minute;
  sc = MyDateTime.Second;
  mnth = MyDateTime.Month;
  Date = MyDateTime.Day;
  yr = MyDateTime.Year;
...
MyDateTime = Clock.read();
     switch (x) {
      ...
      case 2:
        ...
        // show time
        llcd.setCursor(6,1);
        printHrs(MyDateTime.Hour);
        lcd.setCursor(8,1);
        lcd.print(P(":"));
        lcd.setCursor(9,1);
        printMin(MyDateTime.Minute);
        lcd.setCursor(6,2);
        printDate(MyDateTime.Month);
        lcd.setCursor(9,2);
        printDate(MyDateTime.Day);
        lcd.setCursor(12,2);
        lcd.print(MyDateTime.Year);
        ...
        //  set start time 
          SetHour(MyDateTime.Hour);
          clockEdit = false;
          SetMinute(MyDateTime.Minute);
          clockEdit = false;
          SetDate(MyDateTime.Day);
          clockEdit = false;
          clockSelect = true;
          SetMonth(MyDateTime.Month);
          clockEdit = false;
          SetYear(MyDateTime.Year);
          clockEdit = false;
          cleanScreen();
          knob.detachVariable();
          lcd.noCursor();
...
lcd.noBlink();
lcd.setCursor(0,0);
sprintf(buffer, "%2d:%02d:%02d        %3d",MyDateTime.Hour,MyDateTime.Minute, MyDateTime.Second, DSFTemp);
lcd.print(buffer);
lcd.setCursor(19,0);
lcd.print((char)223);
lcd.setCursor(0,1);
...
lcd.print(buffer);
lcd.setCursor(0,3);
sprintf(buffer, "%2d/%02d/%02d ", MyDateTime.Month, MyDateTime.Day, MyDateTime.Year);
lcd.print(buffer);
}  
...
//check & set any time functions
  ...
  minCounter = MyDateTime.Hour * 60 + MyDateTime.Minute;
